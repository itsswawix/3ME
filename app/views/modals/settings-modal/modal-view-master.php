...
function viewMasterData(id) {
    ...
    if (!master) return;
    const typeIcon = { 'Departments': 'building', 'Job Titles': 'briefcase', 'Employment Types': 'clock', 'Leave Types': 'umbrella-beach', 'Performance Periods': 'calendar-alt' }[master.dataType] || 'tag';
    ...
    const content = `
        <style>
            ...
            .modal-view-master { font-family: 'Inter', sans-serif; max-width: 550px; width: 100%; }
            .view-header { display: flex; align-items: flex-start; gap: 16px; margin-bottom: 24px; padding-bottom: 20px; border-bottom: 1px solid #eef2ff; }
            .type-icon-large { width: 64px; height: 64px; border-radius: 20px; background: #4f46e5; display: flex; align-items: center; justify-content: center; color: white; font-size: 1.8rem; }
            .master-info-large h3 { font-size: 1.2rem; font-weight: 600; color: #0f172a; margin-bottom: 4px; }
            .master-meta { font-size: 0.8rem; color: #64748b; display: flex; gap: 8px; flex-wrap: wrap; }
        ...
        <div class="modal-view-master">
            <div class="view-header">
                <div class="type-icon-large"><i class="fas fa-${typeIcon}"></i></div>
                <div class="master-info-large">
                    <h3>${escapeHtml(master.value)}</h3>
                    <div class="master-meta"><span><i class="fas fa-tag"></i> ${escapeHtml(master.dataType)}</span><span><i class="fas fa-hashtag"></i> ${master.id}</span></div>
                    <div style="margin-top: 8px;"><span class="badge ${master.isActive ? 'badge-success' : 'badge-secondary'}">${master.isActive ? 'Active' : 'Inactive'}</span></div>
                </div>
            </div>
            <div class="detail-grid">
                <div class="detail-item full-width"><div class="detail-label"><i class="fas fa-align-left"></i> Description</div><div class="detail-value">${escapeHtml(master.description) || 'No description provided'}</div></div>
            </div>
            <div class="modal-buttons">
                <button type="button" class="btn-secondary" onclick="closeModal()"><i class="fas fa-times"></i> Close</button>
                <button type="button" class="btn-primary" onclick="closeModal(); editMasterData('${id}');"><i class="fas fa-edit"></i> Edit</button>
                <button type="button" class="btn-primary" onclick="duplicateMasterData('${id}');" style="background: #f59e0b;"><i class="fas fa-copy"></i> Duplicate</button>
            </div>
        </div>
    `;
    openModal('Master Data Details', content);
