...
function openEditCompanyModal(company) {
    const content = `
        ...
        <div class="modal-edit-org">
            <div class="org-id-badge"><span>Editing Company:</span><strong>${escapeHtml(company.id)}</strong></div>
...
function openEditDepartmentModal(dept) {
    const content = `
        ...
        <div class="modal-edit-org">
            <div class="org-id-badge"><span>Editing Department:</span><strong>${escapeHtml(dept.id)}</strong></div>
            <form id="editDepartmentForm" onsubmit="updateDepartment(event, '${dept.id}')">
                <div class="form-grid">
                    ...
                    <div class="form-group full-width"><label>Department Head</label><input type="text" id="editDeptHead" value="${escapeHtml(dept.head || '')}"></div>
...
function openEditPositionModal(pos) {
    const content = `
        ...
        <div class="modal-edit-org">
            <div class="org-id-badge"><span>Editing Position:</span><strong>${escapeHtml(pos.id)}</strong></div>
            <form id="editPositionForm" onsubmit="updatePosition(event, '${pos.id}')">
                <div class="form-grid">
                    ...
                    <div class="form-group"><label>Vacancies</label><input type="number" id="editPosVacancies" min="0" value="${pos.vacancies || 0}"></div>
                    <div class="form-group full-width"><label>Reports To</label><input type="text" id="editPosReportsTo" value="${escapeHtml(pos.reportsTo || '')}"></div>
                    
                    <div class="section-title"><i class="fas fa-dollar-sign"></i> Salary Offer</div>
                    <div class="form-group"><label>Minimum Salary (₱)</label><input type="number" id="editPosSalaryMin" min="0" step="0.01" value="${pos.salaryMin || ''}" placeholder="0.00"></div>
                    <div class="form-group"><label>Maximum Salary (₱)</label><input type="number" id="editPosSalaryMax" min="0" step="0.01" value="${pos.salaryMax || ''}" placeholder="0.00"></div>
                    
                    <div class="form-group full-width"><label>Status</label><select id="editPosStatus">${['Active','Inactive'].map(s => `<option value="${s}" ${pos.status === s ? 'selected' : ''}>${s}</option>`).join('')}</select></div>
                </div>
                <div class="modal-buttons">
                    <button type="button" class="btn-cancel" onclick="closeModal()"><i class="fas fa-times"></i> Cancel</button>
                    <button type="submit" class="btn-save"><i class="fas fa-save"></i> Save Changes</button>
...
async function updateDepartment(event, id) {
    ...
    try {
        const response = await fetch('../../api/settings/settings_api.php?action=update_department', {
            ...
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({ id, name, head, status })
        });
...
async function updatePosition(event, id) {
    ...
    const vacancies = document.getElementById('editPosVacancies').value;
    const reportsTo = document.getElementById('editPosReportsTo').value.trim();
    const salaryMin = document.getElementById('editPosSalaryMin').value;
    const salaryMax = document.getElementById('editPosSalaryMax').value;
    ...
    try {
        const response = await fetch('../../api/settings/settings_api.php?action=update_job', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({ 
